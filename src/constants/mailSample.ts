...
export const getMailSample = ({
  ...
  const companyName = useAppSelector(state => state.companyUser.companyNm) || '저희 병원';
  ...
  switch (mailType) {
    ...
    case '서류합격':
      ...
      <p>${applicantName}님께서 정성스럽게 작성해주신 이력서를 보니, 꼭 만나뵙고 싶다는 생각이 들었습니다.</p>
      <p><br></p>
      ...
      <p>- 소요 시간 :</p>
      <p><br></p>
      <p>본 메일은 발신전용으로 회신할 수 없습니다. 문의사항은 병원으로 직접 문의바랍니다. 감사합니다.</p>`;
    case '서류불합격':
      return `
      <p>안녕하세요, ${applicantName}님. </p><p><br></p><p>${companyName}의 ${jobpostTitle} 전형에 지원해주셔서 진심으로 감사합니다. </p><p><br></p><p>보내주신 서류들을 검토한 결과 안타깝게도 ${applicantName}님이 탈락하게 되었습니다. </p><p><br></p><p>우리 병원과 함께 할 가능성을 보여주신 것에 대해 다시 한번 감사드립니다.</p><p><br></p>
      <p>본 메일은 발신전용으로 회신할 수 없습니다. 문의사항은 병원으로 직접 문의바랍니다. 감사합니다.</p>`;
    case '면접합격':
      return `
      <p>안녕하세요, ${applicantName}님.</p>
